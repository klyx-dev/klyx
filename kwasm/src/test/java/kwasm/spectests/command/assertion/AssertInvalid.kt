...
fun List<Token>.parseAssertInvalid(fromIndex: Int): ParseResult<AssertInvalid>? {
    var currentIndex = fromIndex
    if (!isOpenParen(currentIndex)) return null
    currentIndex++
    if (!isKeyword(currentIndex, "assert_invalid")) return null
    currentIndex++
    ...
    return ParseResult(
        AssertInvalid(scriptModule.astNode, message.astNode.value),
