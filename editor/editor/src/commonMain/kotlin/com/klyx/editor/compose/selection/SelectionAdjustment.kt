...
import com.klyx.editor.compose.text.findFollowingBreak
import com.klyx.editor.compose.text.findPrecedingBreak
import com.klyx.editor.compose.text.getParagraphBoundary
...
internal fun interface SelectionAdjustment {
    ...
    companion object {
        ...
        val Word = SelectionAdjustment { layout ->
            adjustToBoundaries(layout) { editorState.getWordBoundary(it) }
        }
...
private fun SelectionLayout.updateSelectionBoundary(
    ...
    val currentRawLine by lazy(LazyThreadSafetyMode.NONE) { info.editorState.cursorAt(currentRawOffset).line }
    ...
    val previousRawLine = info.editorState.cursorAt(rawPreviousHandleOffset).line
    ...
    val previousSelectionWordBoundary =
        info.editorState.getWordBoundary(previousSelectionOffset)
...
private fun SelectableInfo.snapToWordBoundary(
    ...
    val wordBoundary = editorState.getWordBoundary(currentOffset)
    ...
    // same line.
    val wordStartLine = editorState.cursorAt(wordBoundary.start).line
    val start =
        if (wordStartLine == currentLine) {
            wordBoundary.start
        } else if (currentLine >= editorState.lineCount) {
            ...
            editorState.getLineStart(editorState.lineCount - 1)
        } else {
            editorState.getLineStart(currentLine)
        }

    val wordEndLine = editorState.cursorAt(wordBoundary.end).line
    val end =
        if (wordEndLine == currentLine) {
            ...
            editorState.getLineEnd(editorState.lineCount - 1)
        } else {
            editorState.getLineEnd(currentLine)
        }
...
private fun Selection.AnchorInfo.changeOffset(
    ...
    copy(offset = newOffset, direction = info.editorState.getBidiRunDirection(newOffset))
