...
public abstract class GrammarFilesTask extends DefaultTask {
    ...
    /**
     * Get the include directories of the grammar.
     */
    @InputFiles
    @PathSensitive(PathSensitivity.RELATIVE)
    public final File[] getIncludeDirs() {
        return includeDirs;
    }

    /**
     * Set the include directories of the grammar.
     */
    public final void setIncludeDirs(File[] includeDirs) {
    ...
    private void generateJniBinding(Path srcDir) throws GradleException {
        ...
        var template = readResource("jni.c.in")
            .replace("@INCLUDES@", Arrays.stream(includes).map(include -> "#include <" + include + ">").collect(Collectors.joining("\n")))
    ...
    private void generateCmakeLists(Path srcDir) throws GradleException {
        var jniBinding = srcDir.resolve("jni").resolve("binding.c");
        var files = Arrays.stream(grammarFiles)
            .map(file -> toCMakePath(relative(file.toPath())));
        var includeDir = Arrays.stream(includeDirs)
            .map(file -> toCMakePath(relative(file.toPath())));
        var jniBindingPath = toCMakePath(relative(jniBinding));
        var sources = jniBindingPath + " " + String.join(" ", files.toList());

        var includeDirString = String.join(" ", includeDir.toList());

        var template = readResource("CMakeLists.txt.in")
            .replace("@LIBRARY@", libraryName)
            .replace("@INCLUDE@", includeDirString)
    ...
    private static String toCMakePath(Path path) {
