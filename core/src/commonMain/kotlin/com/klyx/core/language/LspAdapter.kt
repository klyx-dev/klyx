...
interface LspAdapter : DynLspInstaller {
    ...
    suspend fun initializationOptions(delegate: LspAdapterDelegate): AnyhowResult<JsonObject?> = Ok(null)
    suspend fun workspaceConfiguration(delegate: LspAdapterDelegate): AnyhowResult<JsonObject> = Ok(emptyJsonObject())

    fun diskBasedDiagnosticSources(): List<String> = emptyList()
    fun diskBasedDiagnosticsProgressToken(): String? = null

    fun languageIds(): HashMap<LanguageName, String> = hashMapOf()
}
    ...
    override fun getLanguageServerCommand(
        ...
        return suspend f@{
            ...
            if (!binaryOptions.allowBinaryDownload) {
                return@f Pair(
                    anyhow.err("downloading language servers disabled"),
                    null
                )
            }
            ...
            val containerDir = delegate.languageServerDownloadDir(name())
                ?: return@f Pair(anyhow.err("no language server download dir defined"), null)

            val lastDownloadedBinary = cachedServerBinary(containerDir, delegate)
                ?.let(::Ok)
                ?: anyhow.err("did not find existing language server binary, falling back to downloading")
