...
actual class Thread(private val javaThread: java.lang.Thread) {
    ...
    actual fun start() = javaThread.start()

    actual fun join() = javaThread.join()

    actual fun join(millis: Long) = javaThread.join(millis)

    actual fun interrupt() = javaThread.interrupt()
