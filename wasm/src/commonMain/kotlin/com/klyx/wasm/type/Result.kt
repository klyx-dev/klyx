...
sealed class Result<out T : WasmType, out E : WasmType> : WasmType {
    data class Ok<T : WasmType>(val value: T) : Result<T, Nothing>() {
        ...
        companion object {
            fun <T : WasmType> reader(okReader: WasmMemoryReader<T>) = run {
    ...
    data class Err<E : WasmType>(val error: E) : Result<Nothing, E>() {
        override fun writeToBuffer(buffer: ByteArray, offset: Int) {
        ...
        companion object {
            fun <E : WasmType> reader(errReader: WasmMemoryReader<E>) = run {
    ...
    inline fun <R : WasmType> map(transform: (T) -> R) = when (this) {
        is Ok -> Ok(transform(value))
        is Err -> this
    }

    inline fun <F : WasmType> mapErr(transform: (E) -> F) = when (this) {
        is Ok -> this
        is Err -> Err(transform(error))
    }

    companion object {
        fun <T : WasmType, E : WasmType> reader(
            ...
            return object : WasmMemoryReader<Result<T, E>> {
                ...
                override fun read(memory: WasmMemory, offset: Int): Result<T, E> {
                    val discriminant = memory.readInt(offset)
...
fun <T : WasmType> Ok(value: T) = Result.Ok(value)
fun <E : WasmType> Err(error: E) = Result.Err(error)
