...
import com.klyx.wasm.internal.writeInt16LittleEndian
import com.klyx.wasm.internal.writeUInt16LittleEndian
import com.klyx.wasm.memory.int16
import com.klyx.wasm.memory.uint16
import kotlin.jvm.JvmInline

@OptIn(ExperimentalWasmApi::class)
@JvmInline
value class WasmUShort(val value: UShort) : WasmType {
    override fun writeToBuffer(buffer: ByteArray, offset: Int) {
        require(offset + SIZE_BYTES <= buffer.size) { "Buffer too small for WasmU16" }
        buffer.writeUInt16LittleEndian(value, offset)
    }
...
@OptIn(ExperimentalWasmApi::class)
@JvmInline
value class WasmShort(val value: Short) : WasmType {
    override fun writeToBuffer(buffer: ByteArray, offset: Int) {
        require(offset + SIZE_BYTES <= buffer.size) { "Buffer too small for WasmS16" }
        buffer.writeInt16LittleEndian(value, offset)
    }
